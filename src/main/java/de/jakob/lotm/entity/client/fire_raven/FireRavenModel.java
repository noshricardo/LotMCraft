...
public class FireRavenModel<T extends FireRavenRenderState> extends EntityModel<T> {
	// This layer location should be baked with EntityRendererProvider.Context in the entity renderer and passed into this model's constructor
	public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(Identifier.fromNamespaceAndPath(LOTMCraft.MOD_ID, "fire_raven"), "main");
	...
	public FireRavenModel(ModelPart root) {
		...
		this.flyAnimation = FireRavenAnimations.FLY.bake(root);
		this.idleAnimation = FireRavenAnimations.IDLE.bake(root);
	}
	...
	public void setupAnim(T state) {
		...
		if (state.isFlying) {
			this.flyAnimation.apply(state.flyAnimationState, state.ageInTicks, 1f);

			float flyingBob = Mth.sin(state.ageInTicks * 0.1f) * 0.05f;
			this.body.y += flyingBob;
		} else {
			this.idleAnimation.apply(state.idleAnimationState, state.ageInTicks, 1f);

			if (state.walkLimbSwingAmount > 0.01f) {
				this.flyAnimation.apply(state.flyAnimationState, state.ageInTicks, 1f);
			}
		}
	}
