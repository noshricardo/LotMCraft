package de.jakob.lotm.entity.custom.ability_entities.projectiles;
...
public class LonginusSpearProjectileEntity extends AbstractArrow {
    ...
    public LonginusSpearProjectileEntity(EntityType<? extends AbstractArrow> entityType, Level level) {
    ...
    public LonginusSpearProjectileEntity(Level level, LivingEntity owner, double damage, boolean griefing, Ability ability) {
        super(ModEntities.LONGINUS_SPEAR.get(), level);
        this.level = level;
        this.owner = owner;
        this.damage = damage;
    ...
    private final DustParticleOptions dust = new DustParticleOptions(
            new Vector3f(139 / 255f, 69 / 255f, 19 / 255f), // saddle brown
            1.5f
    );
    ...
    public void tick() {
        ...
        // Brown dust particles instead of END_ROD
        ParticleUtil.spawnParticles((ServerLevel) level, dust, position(), 8, .3, .3, .3, 0);
    }
    ...
    protected void onHitEntity(EntityHitResult result) {
        ...
        target.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SLOWDOWN, 20 * 3, 7, false, false, false));
        ...
        ServerScheduler.scheduleForDuration(0, 4, 20 * 8, () -> {
            ParticleUtil.spawnSphereParticles(serverLevel, dust, targetPos, 3.5f, 100);
        });
    }
    ...
    protected void onHitBlock(BlockHitResult result) {
        ...
        if (griefing) {
            AbilityUtil.getBlocksInSphereRadius(serverLevel, result.getLocation(), 7, true, true, false).forEach(blockPos -> {
                ParticleUtil.spawnParticles(serverLevel, dust, blockPos.getCenter(), 3, 0.1, 0);
